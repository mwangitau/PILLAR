...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (user && !isUserLoading && !userProfile && !isUserProfileLoading) {
      router.push('/start');
    }
  ...
  const financesQuery = useMemoFirebase(
    () =>
      user
        ? query(
            collection(firestore, `users/${user.uid}/financeRecords`),
            orderBy('date', 'desc'),
            limit(5)
          )
        : null,
  ...
  const { data: transactions, isLoading: financesLoading } =
    useCollection<FinanceRecord>(financesQuery);

  const dashboardImage = placeholderImages.find(
    (p) => p.id === 'dashboard-hero'
  );
  const isLoading =
    isUserLoading ||
    habitsLoading ||
  ...
  const { habitProgress, habitStreak } = useMemo(() => {
    ...
    const habitStreak = habits.reduce(
      (max, h) => ((h.streak || 0) > max ? h.streak : max),
      0
    );
  ...
  if (isUserLoading || (user && !userProfile && !isUserProfileLoading)) {
    return (
    ...
    <AppLayout>
      <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {habits?.filter((h) => h.completed).length || 0} of{' '}
                {habits?.length || 0} habits completed today.
              </p>
            </CardContent>
          </Card>
          <Card>
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-2xl font-bold">{habitStreak} Days</div>
              )}
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Net Finances</CardTitle>
              ...
              {isLoading ? (
                ...
                <div
                  className={`text-2xl font-bold ${
                    netFinances >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}
                >
                  Ksh {netFinances.toLocaleString()}
                </div>
              )}
              <p className="text-xs text-muted-foreground">
                ...
                <Link href="/manual">
                  <Card className="h-full hover:bg-secondary transition-colors">
                    <CardHeader>
                      ...
                      <CardDescription>
                        Review your personalized AI-generated plan.
                      </CardDescription>
                    </CardHeader>
                  </Card>
                </Link>
                <Link href="/habits">
                  <Card className="h-full hover:bg-secondary transition-colors">
                    <CardHeader>
                      ...
                      <CardDescription>
                        Log today's progress and maintain your streaks.
                      </CardDescription>
                    </CardHeader>
                  </Card>
                </Link>
                ...
                <div className="relative rounded-xl overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 p-6">
                    <h3 className="text-2xl font-headline font-bold text-white">
                      New: AI Journal Analysis
                    </h3>
                    <p className="text-white/90 max-w-prose mt-2">
                      Discover deeper insights from your journal entries with
                      our new mood and sentiment analysis feature.
                    </p>
                      ...
                      <div key={i} className="flex items-center gap-4">
                        ...
                        <div className="grid gap-1">
                          <p className="text-sm font-medium leading-none">
                            {habit.completed ? 'Completed' : 'Logged'}: "
                            {habit.name}"
                          </p>
